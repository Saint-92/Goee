...
const MotorhomesModule = {
  ...
  initSwiper() {
    ...
    MOTORHOMES_DATA.images.forEach((image, index) => {
    ...
    MOTORHOMES_DATA.swiper = new Swiper('.motorhomes-swiper', {
      slidesPerView: 1,
      spaceBetween: 0,
      loop: false,
      pagination: {
  ...
  initCustomNavigation() {
    ...
    if (prevBtn && MOTORHOMES_DATA.swiper) {
      prevBtn.addEventListener('click', () => {
        MOTORHOMES_DATA.swiper.slidePrev();
      });
      
      if (MOTORHOMES_DATA.swiper.isBeginning && !MOTORHOMES_DATA.swiper.params.loop) {
        prevBtn.classList.add('swiper-button-disabled');
      }
    }
    
    if (nextBtn && MOTORHOMES_DATA.swiper) {
      nextBtn.addEventListener('click', () => {
        MOTORHOMES_DATA.swiper.slideNext();
      });
      
      if (MOTORHOMES_DATA.swiper.isEnd && !MOTORHOMES_DATA.swiper.params.loop) {
        nextBtn.classList.add('swiper-button-disabled');
      }
    }
    
    if (MOTORHOMES_DATA.swiper) {
      MOTORHOMES_DATA.swiper.on('slideChange', function() {
        if (prevBtn && nextBtn) {
  ...
  initLightbox() {
    ...
    MOTORHOMES_DATA.lightbox = new SimpleLightbox('[data-lightbox="motorhomes-gallery"]', {
      captions: true,
      captionsData: 'title',
    ...
    console.log('SimpleLightbox initialized:', MOTORHOMES_DATA.lightbox);
    ...
    if (lightboxToggle) {
      lightboxToggle.addEventListener('click', (e) => {
        ...
        const currentIndex = MOTORHOMES_DATA.swiper ? MOTORHOMES_DATA.swiper.realIndex : 0;
    ...
    document.querySelectorAll('.swiper-slide img').forEach((img) => {
      ...
      img.addEventListener('click', (e) => {
        ...
        if (link && MOTORHOMES_DATA.lightbox) {
          link.click();
        }
      });
    });
  ...
  initCalendar() {
    ...
    try {
      flatpickrInstance = flatpickr(datepicker, {
        ...
        minDate: "today",
        defaultDate: [
          MOTORHOMES_DATA.selectedDates.start,
          MOTORHOMES_DATA.selectedDates.end
        ],
        onChange: function(selectedDates) {
          if (selectedDates.length === 2) {
            MOTORHOMES_DATA.selectedDates.start = selectedDates[0];
            MOTORHOMES_DATA.selectedDates.end = selectedDates[1];
          }
        },
      });
    ...
    applyDates?.addEventListener('click', () => {
      if (flatpickrInstance) {
        ...
        if (dates.length === 2) {
          MOTORHOMES_DATA.selectedDates.start = dates[0];
          MOTORHOMES_DATA.selectedDates.end = dates[1];
          this.updateSelectedPeriod();
          this.updatePrices();
          closeCalendarModal();
          
          this.updateUrlWithDates(dates[0], dates[1]);
        } else {
  ...
  updateSelectedPeriod() {
    ...
    const start = MOTORHOMES_DATA.selectedDates.start;
    const end = MOTORHOMES_DATA.selectedDates.end;
    
    const startFormatted = Utils.formatDate(start);
    const endFormatted = Utils.formatDate(end);
  ...
  updatePrices() {
    ...
    const dailyPrice = MOTORHOMES_DATA.pricePerDay;
  ...
  generateShareUrl() {
    const shareButton = document.getElementById('shareButton');
    const motorhomeId = shareButton?.getAttribute('data-motorhome-id') || 'adria-s75-sl-2023';
    const startDate = MOTORHOMES_DATA.selectedDates.start;
    const endDate = MOTORHOMES_DATA.selectedDates.end;
    ...
    MOTORHOMES_DATA.shareUrl = url.toString();
    return MOTORHOMES_DATA.shareUrl;
  },
  ...
  initShareButton() {
    ...
    shareButton.addEventListener('click', (e) => {
      ...
      const shareText = `Check out the Adria S75 SL 2023 motorhome! Available from ${Utils.formatDate(MOTORHOMES_DATA.selectedDates.start)} to ${Utils.formatDate(MOTORHOMES_DATA.selectedDates.end)}.`;
      ...
      if (navigator.share) {
        ...
        .catch(error => {
          if (error.name !== 'AbortError') {
            this.openShareMenu(shareUrl, shareText);
          }
        });
      } else {
        this.openShareMenu(shareUrl, shareText);
  ...
  openShareMenu(url, text) {
    ...
    if (shareMenu && shareUrlDisplay) {
      ...
      MOTORHOMES_DATA.shareUrl = url;
      MOTORHOMES_DATA.shareText = text;
    }
  },
  ...
  handleShareAction(action) {
    const url = MOTORHOMES_DATA.shareUrl;
    const text = MOTORHOMES_DATA.shareText || 'Check out this amazing motorhome!';
      ...
      if (startDateParam && endDateParam) {
        try {
          ...
          if (startDate && endDate && startDate <= endDate) {
            MOTORHOMES_DATA.selectedDates.start = startDate;
            MOTORHOMES_DATA.selectedDates.end = endDate;
